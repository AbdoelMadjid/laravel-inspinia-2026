...
class BackupController extends Controller
{
        ...
        foreach ($files as $file) {
            if (pathinfo($file, PATHINFO_EXTENSION) === 'sql') {
                ...
                $backups[] = [
                    ...
                    'size_raw' => $size,
                    'created_at' => \Carbon\Carbon::createFromTimestamp($lastModified)->setTimezone(config('app.timezone', 'Asia/Jakarta')),
                ];
            }
        }
    ...
    public function store(Request $request)
    {
        try {
            ...
            $timestamp = now()->setTimezone(config('app.timezone', 'Asia/Jakarta'))->format('Y-m-d_H-i-s');

            if ($backupType === 'partial' && !empty($selectedTables)) {
                $tableCount = count($selectedTables);
                $filename = 'backup-partial-' . ($dbName ? str_replace(' ', '_', $dbName) : 'db') . '-' . $tableCount . 'tables-' . $timestamp . '.sql';
            } else {
                $filename = 'backup-full-' . ($dbName ? str_replace(' ', '_', $dbName) : 'db') . '-' . $timestamp . '.sql';
            }
    ...
    protected function generateSqlDump(?string $dbName, array $selectedTables = []): string
    {
        ...
        $out .= "-- Date Generated: " . now()->setTimezone(config('app.timezone', 'Asia/Jakarta'))->format('Y-m-d H:i:s T') . "\n";
