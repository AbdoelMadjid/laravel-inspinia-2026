...
@push('styles')
    ...
    <style>
        .dt-processing, #usersTable_processing {
        ...
        .dt-processing .spinner-border, #usersTable_processing .spinner-border {
            position: absolute;
            top: 50%;
            left: 50%;
            margin-top: -0.875rem;
...
<div class="container-fluid">
                ...
                <form id="filterForm" class="row g-2 align-items-center" onsubmit="return false;">
                    <div class="col-lg-3">
                        <div class="app-search">
                            <input type="text" id="customSearchInput" class="form-control" placeholder="Search user name or email..." value="{{ request('search') }}" />
                    ...
                    <div class="col text-end d-flex align-items-center justify-content-end gap-2">
                        <!-- Bulk Action Buttons -->
                        <button type="button" class="btn btn-outline-primary btn-sm d-none" id="bulkAssignRoleBtn" data-bs-toggle="modal" data-bs-target="#bulkAssignRoleModal">
                            <i class="ti ti-user-shield me-1"></i> Role (<span class="selectedUserCount">0</span>)
                        </button>

                        <button type="button" class="btn btn-outline-success btn-sm d-none" id="bulkApproveBtn" onclick="submitBulkApprove()">
                            <i class="ti ti-check me-1"></i> Setujui (<span class="selectedUserCount">0</span>)
                        </button>

                        <button type="button" class="btn btn-outline-danger btn-sm d-none" id="bulkDeleteBtn" onclick="submitBulkDelete()">
    ...
    <div class="d-flex align-items-center gap-1 mb-3">
        <a href="{{ route('admin.users.index') }}" class="btn btn-sm {{ empty($status) ? 'btn-primary' : 'btn-outline-secondary' }}">
            <i class="ti ti-users me-1"></i> Semua Pengguna
        </a>
        <a href="{{ route('admin.users.index', ['status' => 'approved']) }}" class="btn btn-sm {{ $status === 'approved' ? 'btn-success' : 'btn-outline-success' }}">
            <i class="ti ti-circle-check me-1"></i> Disetujui / Aktif
        </a>
        <a href="{{ route('admin.users.index', ['status' => 'pending']) }}" class="btn btn-sm {{ $status === 'pending' ? 'btn-warning' : 'btn-outline-warning' }}">
                            ...
                            <thead class="table-light">
                                <tr>
                                    <th style="width: 35px;" class="text-center">
                                        <input class="form-check-input border-secondary" type="checkbox" id="selectAllUsersHeader" style="border-color: #495057; border-width: 1.5px;">
                                    </th>
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="d-flex align-items-center gap-3">
                            <img src="" id="edit_user_avatar_img" class="rounded-circle avatar-md object-fit-cover" style="width: 48px; height: 48px;" alt="Avatar Preview">
                            ...
                            @foreach($roles as $r)
                                <div class="col-6">
                                    <div class="form-check">
                                        <input class="form-check-input edit-role-checkbox" type="checkbox" name="roles[]" value="{{ $r->name }}" id="edit_role_{{ $r->id }}">
                                        <label class="form-check-label cursor-pointer" for="edit_role_{{ $r->id }}">
                                            {{ ucfirst($r->name) }}
                                        </label>
                                    </div>
                                </div>
            ...
            if (!$.fn.DataTable.isDataTable('#usersTable')) {
                ...
                $(document).on('click', '.edit-user-btn', function () {
                    ...
                    userRoles.forEach(function (roleName) {
                        modal.find('.edit-role-checkbox[value="' + roleName + '"]').prop('checked', true);
                    });
